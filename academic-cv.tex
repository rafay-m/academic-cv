...
\moveleft.5\hoffset\centerline{Paul-Hindemith-Allee 6, App 307, 80939 Munich, Germany}
...
%   \begin{itemize}
%   \item Configure ZED mini2 (RGBD camera) in Jetson TX2, for an additive manufacturing setup that utilizes UR10 robot.
...
%    \item Working on methodology for creating 3D scan of printed specimen for real time monitoring application.
    %\item Using RCNN, trained a classifier by augmenting custom data dataset to segment and size 3D printed specimen in noisy surrounding.
    %\item Design control mechanism for UR10e robot attached to a linear axis in ROS2.
%    \end{itemize}
%\end{itemize}
...
\section{Technical Skills}      
    \begin{description}
   			%\item[OS:] Windows, Linux (Ubunntu).
				...
				%\item[Hardware:] UR-10 robot, Ouster & Livox LiDAR, ZED RGBD Camera
				\item[Other: ] C++,ROS, PyTorch,Git, Docker, OpenFoam, LATEX.
				% \item[Languages:] LATEX, Markdown.
    \end{description}
